...
function renderCatalogo(data) {
    ...
    if (isUnavailable) {
      detailEl.innerHTML = `
        <div class="product-detail">
          ...
          <p class="agotado" style="color:red; font-weight:bold; font-size:1.5rem; text-align:center;">¡Producto agotado!</p>
  ...
  .then(data => {
    ...
    // 🔹 Recargar el detalle del producto (por ejemplo, volver a pedir los embeddings)
    loadProductDetail(product.id); // RESOLVER ERROR DE FUNCION
  })
  .catch(err => console.error('Error registrando vista:', err));
